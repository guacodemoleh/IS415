...
To form the join in three steps: - Convert the *groups* list object into a matrix - Append the *groups* matrix to `shan_sf` using *cbind()* - Use *rename()* from **dplyr** to rename the *as.amtrix.groups* field as `CLUSTER`
...
<to fill in explanations after reading in detail>
