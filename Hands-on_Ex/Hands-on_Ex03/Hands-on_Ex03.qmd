...
```{r}
pacman::p_load(sf, tmap, spatstat, raster,sp)
```

If you are still interested in using maptools (though it has been deprecated),
```{r}
...
Let's use [*as.ppp()*](https://search.r-project.org/CRAN/refmans/spatstat.geom/html/as.ppp.html) of *spatstat* to convert the sf into object format.
...
Let's categorise and extract the planning areas of interest.
```{r}
...
tm_owin <- as.owin(tm_sp)
ck_owin <- as.owin(ck_sp)
jw_owin <- as.owin(jw_sp)
```
...
We can infer from the R value (Nearest Neighbour Index) that since R = 0.60568 < 1, the pattern exhibits clustering in Tampines.
