...
#| eval: false
hex_grid_bounded2 <- hex_grid_bounded2 %>%
  select(SUBZONE_N, everything())

tmap_mode("view")
tm_shape(hex_grid_bounded2) +
  tm_polygons(col = "SUBZONE_N", style = "cat") +
...
```{r}
od_wkday <- odbus_combined %>%
  filter(DAY_TYPE == "WEEKDAY") %>%
  group_by(ORIGIN_PT_CODE, DESTINATION_PT_CODE,TIME_PER_HOUR) %>%
  summarise(TRIPS = sum(TOTAL_TRIPS)) %>%
  ungroup()
```
...
```{r}
#| eval: false
od_data_wkend <- left_join(od_wkend, BusStop_hex,
            by = c("ORIGIN_PT_CODE" = "BusStopCode")) %>%
  rename("ORIGIN_hex" = "index")

od_data_wkend <- left_join(od_data_wkend, BusStop_hex,
            by = c("DESTINATION_PT_CODE" = "BusStopCode")) %>%
  rename("DESTIN_hex" = "index") %>%
  drop_na() %>%
  group_by(ORIGIN_hex, DESTIN_hex, TIME_PER_HOUR) %>%
  summarise(TOTAL_TRIPS = sum(TRIPS))

cols_to_convert <- c("ORIGIN_hex", "DESTIN_hex")

od_data_wkend[cols_to_convert] <- lapply(od_data_wkend[cols_to_convert], as.factor)
```
...
It is not meaningful to include trips that started and ended from the same point. 
...
propulsive <- hex_grid_bounded3 %>%
  st_drop_geometry() %>%
  select(index, biz_count, school_count, fin_count, hc_count, busstop_count, housing_count, leisure_recre_count, retail_count, entertn_count)
...
```{r}
flow_data <- read_rds("data/rds/flow_data")
```
...
Using schools as an example, I will use `flowLine` since it is more typical for passengers to travel to school on weekdays.

::: panel-tabset
### Origin
Origin: Weekday 6pm, School Hexagon Density of 3.

```{r}
#| echo: false
tmap_mode("plot")

sch_hex_3 <- hex_grid_bounded3 %>% filter(school_count == 3)

flowLine_org_sch_3 <- filter(flowLine, ORIGIN_hex %in% sch_hex_3$index)
...
tm_shape() +
  tm_lines(lwd = "TOTAL_TRIPS",
           ...
           alpha = 0.5,
           title.lwd = "Total Trips") + 
  tm_layout(main.title = "Origin School Density: Desire Lines with at least 5000 trips \nbetween Traffic Analysis Zones for Weekday 6pm \n(Nov '23 - Jan '24)",
            main.title.position = "center",
            main.title.size = 0.6,
            frame = TRUE) +
  tm_legend(position = c("RIGHT", "BOTTOM"), legend.text.size = 0.4) 
...
Weekday 7am, School Hexagon Density of 3.
...
flowLine_des_sch_3 <- filter(flowLine, DESTIN_hex %in% sch_hex_3$index)
...
tm_shape() +
  tm_lines(lwd = "TOTAL_TRIPS",
           ...
           alpha = 0.5,
           title.lwd = "Total Trips") + 
  tm_layout(main.title = "Destination School Density: Desire Lines with at least 5000 trips \nbetween Traffic Analysis Zones for Weekday 7am \n(Nov '23 - Jan '24)",
            main.title.position = "center",
            main.title.size = 0.6,
            frame = TRUE) +
  tm_legend(position = c("RIGHT", "BOTTOM"), legend.text.size = 0.4)
...
## Entertainment
We can also have a look at entertainment.

::: panel-tabset

### Origin

Weekday 5pm, Entertainment Hexagon Density.

```{r}
#| echo: false
tmap_mode("plot")

etn_hex <- hex_grid_bounded3 %>% filter(entertn_count >0)

flowLine_org_etn <- filter(flowLine, ORIGIN_hex %in% etn_hex$index)
...
tm_shape(etn_hex) + 
  ...
  tm_borders(col = "grey") +
flowLine_org_etn %>%  
  filter(DAY_TYPE == "WEEKDAY", TOTAL_TRIPS >= 5000, TIME_PER_HOUR == 17) %>%
tm_shape() +
  tm_lines(lwd = "TOTAL_TRIPS",
           style = "fixed",
           scale = c(1,2,3,4,5,7,9),
           ...
           alpha = 0.5,
           title.lwd = "Total Trips") + 
  tm_layout(main.title = "Origin Entertainment Density: Desire Lines with at least 5000 trips \nbetween Traffic Analysis Zones for Weekday 5pm \n(Nov '23 - Jan '24)",
            main.title.position = "center",
            main.title.size = 0.6,
            frame = TRUE) +
  tm_legend(position = c("RIGHT", "BOTTOM"), legend.text.size = 0.4) 
...
Weekend 1pm, Entertainment Hexagon Density.
...
flowLine_des_etn <-filter(flowLine, DESTIN_hex %in% etn_hex$index)
...
tm_shape(etn_hex) + 
  ...
  tm_borders(col = "grey") +
flowLine_des_etn %>%  
  filter(DAY_TYPE == "WEEKENDS/HOLIDAY", TOTAL_TRIPS >= 500, TIME_PER_HOUR == 13) %>%
  tm_shape() +
  tm_lines(lwd = "TOTAL_TRIPS",
           style = "fixed",
           scale = c(1,2,3,4,5,7,9),
           ...
           alpha = 0.5,
           title.lwd = "Total Trips") + 
  tm_layout(main.title = "Entertainment Density: Desire Lines with at least 500 trips \nbetween Traffic Analysis Zones for Weekend 1pm \n(Nov '23 - Jan '24)",
            main.title.position = "center",
            main.title.size = 0.6,
            frame = TRUE) +
  tm_legend(position = c("RIGHT", "BOTTOM"), legend.text.size = 0.4) 
