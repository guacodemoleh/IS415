...
#| eval: false
#| echo: false
top_hexagons <- st_join(flowLine_org, org_hex_3000, 
                        left_on = "ORIGIN_hex", right_on = "index") %>%
  filter(TOTAL_TRIPS >= 3000) 

write_rds(top_hexagons, "data/rds/org_hex_3000_detailed")
```

```{r}
#| eval: false
#| echo: false
org_hex_3000_detailed <- read_rds("data/rds/org_hex_3000_detailed")

aggregated <- org_hex_3000_detailed %>%
  group_by(index) %>%
  summarise(SUBZONE_N = first(SUBZONE_N))

top_hexagons <- org_hex_3000_detailed %>%
  select(ORIGIN_hex, PLN_AREA_N, SUBZONE_N, TOTAL_TRIPS) %>%
  arrange(desc(TOTAL_TRIPS)) %>%
  head(5)

kable(top_hexagons, caption = "Top 5 Most Propulsive Hexagons", row.names = FALSE) %>%
...
dest_hex_8000 <- dest_hex[dest_hex$index %in% flowLine_dest$DESTIN_hex[flowLine_dest$TOTAL_TRIPS >= 8000], ]
